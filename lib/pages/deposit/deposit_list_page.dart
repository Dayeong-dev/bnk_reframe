...
class _DepositListPageState extends State<DepositListPage>
  ...
  // =========================================================
  // (변경) 상단 컨트롤: ① 카테고리(한줄) ② [좌]검색결과 [우]정렬텍스트+단일토글
  // =========================================================
  Widget _topControls(int totalCount) {
    return Column(
      ...
      children: [
        ...
        // ② 한 줄: [좌]검색 결과(+검색어)  [우]정렬 텍스트 + 단일 토글
        Padding(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 10),
              _singleViewToggle(), // 리스트/그리드 토글(아이콘 1개)
            ],
          ),
        ),
      ],
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 금리 라인: 왼쪽 최고, 오른쪽 기본(왼쪽 정렬해서 자연스럽게)
                    Row(
                      children: [
                        _RatePill(
...
class SmartProductBadge extends StatelessWidget {
  ...
  // 2) 안정적인 색 (productId/이름 seed → 항상 동일한 계열)
...
class EmojiThumb extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      alignment: Alignment.center,
      // 이모지는 텍스트라서 기기별 폰트에 따라 크기가 조금 달라요 → *0.62가 대체로 안정적
      child: Text(
        emoji,
...
/// 원형 아이콘 배지: 상품명/카테고리/퍼포즈(해시태그)를 분석해
/// 적절한 머티리얼 아이콘과 색을 자동으로 선택해줍니다.
/// 에셋 불필요, 어느 화면에서도 그대로 사용 가능.
class RoundProductIcon extends StatelessWidget {
  ...
  _IconMeta _pickMeta(DepositProduct p) {
    final text = _mix(p); // 소문자 단일 문자열
    // 1) 제품별 override가 있으면 우선 적용 (원하면 map에 추가)
  ...
  // 제품별 강제 매핑(선택): {productId: (icon, color)}
  static const Map<String, (IconData, Color)> _overrideById = {
    // 예) 'LOTTE_UPGRADE': (Icons.sports_baseball_rounded, Color(0xFFF4511E)),
  };
